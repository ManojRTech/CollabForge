...
const Dashboard = () => {
  ...
  const handleSaveProfile = async () => {
    try {
      ...
      const res = await axios.put("/api/user/me", payload, { headers: { Authorization: `Bearer ${token}` } });
      setUser(res.data.user);
      setMessage("Profile updated successfully!");
      setCurrentPassword(""); setNewPassword(""); setShowPasswordFields(false);
  ...
  const handleAddTask = async () => {
    try {
      const token = localStorage.getItem("token");
      const res = await axios.post("/api/tasks", newTask, { headers: { Authorization: `Bearer ${token}` } });
  ...
  const handleUpdateTask = async () => {
    try {
      ...
      const res = await axios.put(`/api/tasks/${id}`, { title, description, category, deadline, status }, { headers: { Authorization: `Bearer ${token}` } });
    ...
    } catch (err) { setMessage(err.response?.data?.message || "Error updating task"); }
  };
  ...
  const handleAcceptTask = async (id) => {
    try {
      const token = localStorage.getItem("token");
      const res = await axios.post(`/api/tasks/${id}/accept`, {}, { headers: { Authorization: `Bearer ${token}` } });
      setTasks(tasks.map(t => t.id === id ? res.data.task : t));
      setMessage("Task accepted!");
    } catch (err) { setMessage(err.response?.data?.message || "Error accepting task"); }
  };
      ...
      {user && (
        <>
          ...
          <ProfileSection
            ...
            showPasswordFields={showPasswordFields} togglePasswordFields={togglePasswordFields}
            currentPassword={currentPassword} setCurrentPassword={setCurrentPassword}
            newPassword={newPassword} setNewPassword={setNewPassword}
            handleSaveProfile={handleSaveProfile}
          />
          <TaskSection
            newTask={newTask} setNewTask={setNewTask} editingTaskId={editingTaskId}
            handleAddTask={handleAddTask} handleUpdateTask={handleUpdateTask}
            filterCategory={filterCategory} setFilterCategory={setFilterCategory}
            filterStatus={filterStatus} setFilterStatus={setFilterStatus}
            sortByDeadline={sortByDeadline} setSortByDeadline={setSortByDeadline}
            displayedTasks={displayedTasks} user={user}
            handleEditTask={handleEditTask} handleDeleteTask={handleDeleteTask}
            handleAcceptTask={handleAcceptTask} handleStatusChange={handleStatusChange}
